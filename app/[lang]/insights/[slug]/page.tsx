...
export async function generateStaticParams() {
  return await fetchAllPostSlugsWithLang();
}
...
export async function generateMetadata({
  ...
  const canonicalPath =
    currentLang === defaultLanguage
      ? `/insights/${post.slug}`
      : `/${currentLang}/insights/${post.slug}`;
...
export default async function Page({
  ...
  // Fetch strictly for the requested language (supports resolving from master English slug)
  const [settings, post] = await Promise.all([
    fetchSiteSettings(currentLang),
    fetchPostBySlug(decodedSlug, currentLang, true),
  ]);
