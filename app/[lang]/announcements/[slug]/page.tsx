...
export async function generateStaticParams() {
  return await fetchAllPostSlugsWithLang('announcement');
}
...
export async function generateMetadata({
  ...
  const canonicalPath =
    currentLang === defaultLanguage
      ? `/announcements/${post.slug}`
      : `/${currentLang}/announcements/${post.slug}`;
...
export default async function AnnouncementSlugPage({
  params,
}: {
  params: Promise<{ lang: string; slug: string }>;
}) {
