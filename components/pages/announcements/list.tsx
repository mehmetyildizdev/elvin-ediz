            ...
            {announcementPosts.length > 0 && (
              <span className="text-text-muted text-xs">
                {announcementPosts.length > ITEMS_PER_PAGE
                  ? `Showing ${startIndex + 1}–${Math.min(startIndex + ITEMS_PER_PAGE, announcementPosts.length)} of ${announcementPosts.length} announcements`
                  : `Showing ${announcementPosts.length} ${announcementPosts.length === 1 ? 'announcement' : 'announcements'}`}
              </span>
            )}
          </div>
                          ...
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <span className="text-accent block text-[10px] font-bold tracking-widest uppercase">
                                {post.announcementNoticeEyebrow || 'Official Notice'}
                              </span>
                ...
                <div className="border-border-subtle bg-bg-surface rounded-sm border border-dashed p-12 text-center">
                  ...
                  <p className="text-text-muted mt-1 text-xs">
                    {insightsPageData.announcementsEmptyMessage ||
                      'Check back regularly for official IRCC policy notices and immigration advisory updates.'}
                  </p>
                </div>
              )}
            </div>
