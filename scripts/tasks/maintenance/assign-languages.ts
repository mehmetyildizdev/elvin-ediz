...
export const assignLanguagesTask: Task = {
  ...
  description: 'Audits all CMS documents and assigns language: "en" to any document missing a language tag.',
  run: async (ctx: TaskContext) => {
    ...
    const documents = await ctx.client.fetch<Array<{ _id: string; _type: string; title?: string; name?: string }>>(
      `*[${filter}]{ _id, _type, title, name, language }`,
      { types: TARGET_TYPES }
    );
